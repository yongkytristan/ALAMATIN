...
- `data/processed/jabar-reference-v1-verified.json` contains only the 2,876
  three-source consensus rows and can be loaded by `ReferenceHierarchy` for
  safe exact/ambiguous lookup;
- `data/processed/jabar-reference-v1-exceptions.csv` contains the 1,974
  corroborated candidates and 1,107 unresolved rows; and
...
`candidate_review_only` may be shown to a reviewer but never used for automatic
correction. `unresolved_do_not_guess` must remain blank in the accepted postal
field. A held Pos Indonesia observation is retained in separate evidence
columns and cannot change those states automatically.
