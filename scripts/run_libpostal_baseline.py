#!/usr/bin/env python3
"""Run the ALM-016 libpostal NER baseline and report metrics + latency."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not examples:
            raise BaselineRunError(
                f"{args.dataset} has no examples"
            )
        ...
        for example in examples:
            ...
            if not valid:
                raise BaselineRunError(
                    f"{example.get('id', '?')}: "
                    f"baseline produced invalid BIO: {reason}"
                )

            gold_sequences.append(example["labels"])
            predicted_sequences.append(predicted)

        overall = entity_metrics(
            gold_sequences,
            predicted_sequences,
        )

        by_type = entity_metrics_by_type(
            gold_sequences,
            predicted_sequences,
        )
    ...
    except (OSError, KeyError, RuntimeError, BaselineRunError) as error:
